...
def exp_evaluate(args,
    ...
    envs = make_vec_envs(env_name,
                         ...
                         ret_rms=ret_rms,
                         tasks=None,
                         add_done_info=args.max_rollouts_per_task > 1, sparse=args.sparse_reward
                         )
    num_steps = envs._max_episode_steps

    # reset environments
    state, belief, task = utl.reset_env(envs, args, tasks=list(tasks))
    ...
    if encoder is not None:
        ...
        if args.vae == 'gmvae':
            cluster = envs.get_cluster()
            cluster = F.one_hot(torch.tensor(cluster).squeeze(), 4).to(device)
            cluster = cluster.unsqueeze(0)
            latent_sample, latent_mean, latent_logvar, hidden_state, _, _, _ = encoder.prior(num_processes, test=False, cluster=cluster)
        else:
    ...
    _, _, cluster_dist, _ = encoder.get_cluster(actions=action.float(),
                                                            states=state,
                                                            rewards=rew_raw,
                                                            hidden_state=hidden_state)
        ...
        for step_idx in range(num_steps):
            ...
            if encoder is not None:
                ...
                latent_sample, latent_mean, latent_logvar, hidden_state = utl.update_encoding(encoder=encoder,
                                                                                              ...
                                                                                              hidden_state=hidden_state,
                                                                                                test=True, 
                                                                                                cluster=cluster, use_prior=False)
            ...
            if np.sum(done) > 0:
                ...
                state, belief, task = utl.reset_env(envs, args, indices=done_indices, state=state, tasks=list(tasks))

    envs.close()
    return returns_per_episode[:, :num_episodes]


def cluster_confusion(Y, Y_pred):
    ...
    Y_pred_new = [col[i] for i in Y_pred]
    conf = confusion_matrix(Y, Y_pred_new)
    return conf
...
def visualise_behaviour(args,
    ...
    env = make_vec_envs(env_name=args.env_name,
                        ...
                        tasks=tasks, sparse=args.sparse_reward
                        )
