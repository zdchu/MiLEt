...
class OnlineStorage(object):
    def __init__(self,
        ...
        if self.args.pass_latent_to_policy:
            ...
            if args.stack_rnn:
                self.hidden_states = (torch.zeros(num_steps + 1, num_processes, hidden_size), 
                                        torch.zeros(num_steps + 1, num_processes, hidden_size))
            else:
                self.hidden_states = torch.zeros(num_steps + 1, num_processes, hidden_size)
        ...
        self.reward_ent = torch.zeros(num_steps, num_processes, 1)
    ...
    def to_device(self):
        ...
        if self.args.pass_latent_to_policy:
            ...
            if self.args.stack_rnn:
                self.hidden_states = (self.hidden_states[0].to(device), 
    ...
    def insert(self,
        ...
        if self.args.pass_latent_to_policy:
            ...
            if self.args.stack_rnn:
                self.hidden_states[0][self.step + 1].copy_(hidden_states[0].squeeze(0).detach())
                self.hidden_states[1][self.step + 1].copy_(hidden_states[1].squeeze(0).detach())
            else:
                self.hidden_states[self.step + 1].copy_(hidden_states.squeeze(0).detach())
        self.actions[self.step] = actions.detach().clone()
        self.rewards_raw[self.step].copy_(rewards_raw)

        if rewards_ent is not None:
            self.reward_ent[self.step].copy_(rewards_ent)
    ...
    def after_update(self):
        ...
        if self.args.pass_latent_to_policy:
            ...
            if self.args.stack_rnn:
                self.hidden_states[0][0].copy_(self.hidden_states[0][-1])
                self.hidden_states[1][0].copy_(self.hidden_states[1][-1])
            else:
    ...
    def compute_returns(self, next_value, use_gae, gamma, tau, use_proper_time_limits=True):
        ...
        if self.args.explore:
            reward_ent = self.reward_ent.clone().to(device)
            rewards += reward_ent
